# Functionality for cleaning / transforming raw data
...
def get_site_coordinates(tx_id, df_sites):
    """
    ...
    match = re.match(r"^(.*?)_?(?:primary_)?(?:local_)?(?:grid_)?\d+(\.\d+)?kv_t\d+[a-zA-Z]*", tx_id, re.IGNORECASE)
    if not match:
    ...
    potential_rows = df_sites[df_sites['SiteName'].astype(str).str.contains(regex_pattern, regex=True, na=False)]
    ...
    if pd.notna(row['Easting']) and pd.notna(row['Northing']):
        ...
        logging.info("Coordinates found and converted for %s: Lat %.4f, Lon %.4f", tx_id, lat, lon)
        return {'latitude': lat, 'longitude': lon}

    return None


def process_single_site_power(tx_id, df_power_full, start_dt, end_dt):
    """
    ...
    filtered_series = power_series[(power_series.index >= start_dt) & (power_series.index <= end_dt)]

    if filtered_series.empty:
        logging.warning("Power data for %s is empty after date filtering.", tx_id)
        return None

    logging.info("Processed active power for %s (%d hourly records).", tx_id, len(filtered_series))
    return filtered_series


def get_site_era5_data(coords, ds_era5, start_dt, end_dt):
    """
    ...
    time_coord = next((name for name in ['time', 'valid_time'] if name in ds_era5.coords), None)
    ...
    ds_site = ds_era5.sel(
        {lat_coord: coords['latitude'], lon_coord: coords['longitude']}, method='nearest'
    ).sel(
        {time_coord: slice(era5_start_naive, era5_end_naive)}
    ).load()

    if ds_site[time_coord].size == 0:
        logging.warning("No ERA5 data found for the site after time/location selection.")
        return None

    logging.info("Loaded ERA5 data for coordinates (timeseries length: %d).", ds_site[time_coord].size)
...
def create_master_dataframe(target_ids, df_power, df_sites, ds_era5, start_date, end_date, era5_vars):
    """
    ...
    end_dt_utc = pd.to_datetime(end_date, utc=True).replace(hour=23, minute=59, second=59)

    for tx_id in target_ids:
        logging.info("Processing site: %s", tx_id)

        power_series = process_single_site_power(tx_id, df_power, start_dt_utc, end_dt_utc)
        ...
        if df_era5_site is not None:
            df_era5_site = df_era5_site.drop(columns=['latitude', 'longitude'], errors='ignore')
    ...
    if not all_site_dfs:
        logging.error("No data could be processed for any target site. Master DataFrame is empty.")
        return pd.DataFrame()

    master_df = pd.concat(all_site_dfs).reset_index()
    logging.info("Master DataFrame created with %d total records for %d sites before NaN handling.", len(master_df), len(all_site_dfs))
